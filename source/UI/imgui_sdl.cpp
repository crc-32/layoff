...
{
	int x = static_cast<int>(round(u * (Surface->w - 1) + 0.5f));
	int y = static_cast<int>(round(v * (Surface->h - 1) + 0.5f));
...
ImVec2 Texture::Size() { return ImVec2(Surface->w, Surface->h); }
...
namespace ImGuiSDL
{
	...
	Texture* LoadTexture(const std::string &Path)
	{
		...
		res->Source = SDL_CreateTextureFromSurface(CurrentDevice->Renderer, res->Surface);
		return res;
	}
	...
	Texture* LoadTexture(const std::vector<unsigned char> &data)
	{
		...
		Texture *res = new Texture();
		res->Surface = surf;
		res->Source = SDL_CreateTextureFromSurface(CurrentDevice->Renderer, surf);
