...
namespace layoff::IPC::qlaunch {
	namespace {
		...
		template<typename T>
		static inline u32 PunToU32(T value)
		{
			u32 val = 0;
		...
		static inline Result SendMessage(GeneralChannelMessage message, T ...args)
		{
			...
			if (R_SUCCEEDED(rc))
			{
				...
				u32 buf[1012 / 4] = { 1, PunToU32(args)... };
	...
	{
		return SendMessage(GeneralChannelMessage::OverlayOpened);
	}

	Result SignalOverlayClosed()
	{
		return SendMessage(GeneralChannelMessage::OverlayHidden);
	}
}